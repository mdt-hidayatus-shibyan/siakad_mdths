...
<head>
    ...
    <title>Ijazah_Alquran_{{ Str::slug($peserta->murid->nama_lengkap ?? 'Murid') }}</title>
...
<body>
    ...
    <div
        ...
        <div
            class="px-3 py-1.5 bg-emerald-500/20 text-emerald-300 border border-emerald-400/40 rounded-lg text-xs font-bold flex items-center shadow-2xs">
            📖 IJAZAH LULUS AL-QUR'AN (DOKUMEN RESMI)
        </div>
    ...
    <div class="ijazah-wrapper">
        ...
        <div class="title-section">
            ...
            <div class="title-tahun">Tahun Pelajaran
                {{ $peserta->ujianAlquran->tahunPelajaran->nama_masehi ?? ($data['tahun_masehi'] ?? '-') }} M
                / {{ $peserta->ujianAlquran->tahunPelajaran->nama_hijriyah ?? ($data['tahun_hijriyah'] ?? '-') }} H
            </div>
        </div>
        ...
        <div class="content-section">
            ...
            <table class="tabel-data">
                <tr>
                    ...
                    <td class="value-col uppercase">{{ $peserta->murid->nama_lengkap ?? ($data['nama_murid'] ?? '-') }}
                    </td>
                </tr>
                <tr>
                    ...
                    <td class="separator-col">:</td>
                    <td class="value-col">
                        {{ $peserta->murid->tempat_lahir ?? ($data['tempat_lahir'] ?? '-') }},
                        {{ !empty($peserta->murid->tanggal_lahir) ? \Carbon\Carbon::parse($peserta->murid->tanggal_lahir)->translatedFormat('d F Y') : (!empty($data['tanggal_lahir']) ? \Carbon\Carbon::parse($data['tanggal_lahir'])->translatedFormat('d F Y') : '-') }}
                    </td>
                </tr>
                <tr>
                    ...
                    <td class="separator-col">:</td>
                    <td class="value-col">
                        {{ $peserta->murid->waliMurid->nama_lengkap ?? ($peserta->murid->nama_ayah ?? ($data['nama_wali'] ?? '-')) }}
                    </td>
                </tr>
                <tr>
                    <td class="label-col">Nomor Induk / NISM</td>
                    <td class="separator-col">:</td>
                    <td class="value-col tracking-widest">{{ $peserta->murid->nism ?? ($data['nism'] ?? '-') }}</td>
                </tr>
            </table>
            ...
            <p>
                ...
                <strong>{{ !empty($peserta->ujianAlquran->tanggal_ujian) ? \Carbon\Carbon::parse($peserta->ujianAlquran->tanggal_ujian)->translatedFormat('d F Y') : (!empty($peserta->ujianAlquran->tanggal_pelaksanaan) ? \Carbon\Carbon::parse($peserta->ujianAlquran->tanggal_pelaksanaan)->translatedFormat('d F Y') : (!empty($peserta->tanggal_lulus) ? \Carbon\Carbon::parse($peserta->tanggal_lulus)->translatedFormat('d F Y') : (!empty($data['tanggal_ujian']) ? \Carbon\Carbon::parse($data['tanggal_ujian'])->translatedFormat('d F Y') : '-'))) }}</strong>.
            </p>
            ...
            <div class="text-right mt-4" style="padding-right: 1.5cm;">
                Bangkalan,
                {{ !empty($peserta->tanggal_lulus) ? \Carbon\Carbon::parse($peserta->tanggal_lulus)->translatedFormat('d F Y') : (!empty($data['tanggal_disahkan']) ? \Carbon\Carbon::parse($data['tanggal_disahkan'])->translatedFormat('d F Y') : now()->translatedFormat('d F Y')) }}
            </div>
        </div>
        ...
        <div class="footer-section">
            ...
            <div class="ttd-box">
                ...
                <p class="ttd-nama">{{ $juriPj?->ustadz?->nama_lengkap ?? ($data['juri_pj_nama'] ?? 'Dewan Penguji') }}
                </p>
            </div>
            ...
            <div class="ttd-box">
                ...
                <p class="ttd-nama">
                    {{ $pengasuh?->anggota?->nama_lengkap ?? ($pengasuh?->nama ?? ($data['pengasuh_nama'] ?? 'Pengasuh')) }}
                </p>
            </div>
        ...
        <div class="nomor-bawah">
            No Ijazah :
            {{ $peserta->nomor_ijazah_alquran ?? ($peserta->no_ijazah ?? ($data['nomor_dokumen'] ?? '-')) }}
        </div>
