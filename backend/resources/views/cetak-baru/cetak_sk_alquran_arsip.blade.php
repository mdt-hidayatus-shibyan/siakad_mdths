...
<head>
    ...
    <title>
        SK_Kelulusan_Alquran_{{ $data['status_kelulusan'] ?? 'Hasil' }}_{{ Str::slug($data['nama_murid'] ?? 'Murid') }}
    </title>
...
<body>
    ...
    <div
        ...
        <div
            ...
            🛡️ SURAT KEPUTUSAN RESMI UJIAN AL-QUR'AN (ARSIP RESMI)
        </div>
    ...
    <div class="sk-container text-[10.5pt] text-justify">
        ...
        <div class="judul-sk">
            ...
            <p class="font-bold text-[10pt]">Nomor:
                {{ $data['nomor_sk_alquran'] ?? ($data['nomor_dokumen'] ?? 'SK/UAQ/' . date('Y')) }}
            </p>
                ...
                <td>
                    <ol class="list-decimal pl-4 m-0">
                        ...
                        <li>Bahwa berdasarkan hasil evaluasi Dewan Juri pada Ujian Al-Qur'an Tahun Pelajaran
                            {{ $data['tahun_masehi'] ?? date('Y') }}, murid yang bersangkutan
            ...
            <tr>
                ...
                <td>
                    <ol class="list-decimal pl-4 m-0">
                        ...
                        <li>Ketetapan Batas Minimal Nilai Kelulusan (KKM &ge; 55).</li>
                    </ol>
                </td>
            </tr>
            <tr>
        ...
        <table class="tabel-identitas">
            ...
            <tr>
                <td>Ruangan / Kelas</td>
                <td>:</td>
                <td>{{ $data['nama_ruangan'] ?? '-' }} (Level: {{ $data['nama_level'] ?? '5 IBT' }})</td>
            </tr>
            <tr>
                <td>Nomor Peserta</td>
                <td>:</td>
        ...
        <table class="tabel-rincian">
            ...
            <tbody>
                <tr class="font-semibold">
                    <td>{{ $data['jumlah_khoto_jali'] ?? 0 }} x 5 =
                        -{{ $data['poin_pengurangan_jali'] ?? ($data['jumlah_khoto_jali'] ?? 0) * 5 }}</td>
                    <td>{{ $data['jumlah_khoto_khofi'] ?? 0 }} x 3 =
                        -{{ $data['poin_pengurangan_khofi'] ?? ($data['jumlah_khoto_khofi'] ?? 0) * 3 }}</td>
                    <td class="text-rose-700 font-bold">-{{ $data['total_pengurangan'] ?? 0 }}</td>
                    <td class="font-black text-[11pt]">{{ number_format($data['nilai_akhir'] ?? 0, 0) }} / 100</td>
                    <td>{{ $data['predikat'] ?? '-' }} ({{ $data['predikat_arab'] ?? '-' }})</td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="text-center my-2">
            <span
                class="inline-block px-8 py-1 border-2 border-black font-black text-[12pt] uppercase tracking-widest {{ ($data['status_kelulusan'] ?? 'Lulus') === 'Lulus' ? 'bg-slate-100' : 'bg-rose-100 text-rose-800' }}">
                --- {{ strtoupper($data['status_kelulusan'] ?? 'LULUS') }} ---
            </span>
        </div>
        ...
        <table class="w-full tabel-konsideran mb-2">
            <tr>
                ...
                <td>:</td>
                <td>
                    @if (($data['status_kelulusan'] ?? 'Lulus') === 'Lulus')
            ...
            <tr>
                ...
                <td class="w-1/3 align-top" style="height: 55px;">
                    ...
                    <p class="mb-0">Tanggal :
                        {{ !empty($data['tanggal_ujian']) ? \Carbon\Carbon::parse($data['tanggal_ujian'])->translatedFormat('d F Y') : (!empty($data['tanggal_pelaksanaan']) ? \Carbon\Carbon::parse($data['tanggal_pelaksanaan'])->translatedFormat('d F Y') : (!empty($data['tanggal_lulus']) ? \Carbon\Carbon::parse($data['tanggal_lulus'])->translatedFormat('d F Y') : (!empty($data['tanggal_disahkan']) ? \Carbon\Carbon::parse($data['tanggal_disahkan'])->translatedFormat('d F Y') : now()->translatedFormat('d F Y')))) }}
                    </p>
            ...
            <tr>
                <td class="align-bottom">
                    <p class="font-bold underline mb-0">
                        {{ $data['pengasuh_nama'] ?? 'Pengasuh Madrasah' }}</p>
                </td>
                <td class="align-bottom">
                    <p class="font-bold underline mb-0">
                        {{ $data['kabid_nama'] ?? 'Kepala Bidang' }}</p>
                </td>
                <td class="align-bottom">
                    <p class="font-bold underline mb-0">
                        {{ $data['juri_pj_nama'] ?? ($data['admin_nama'] ?? 'Dewan Penguji') }}
                    </p>
                </td>
            </tr>
        </table>
