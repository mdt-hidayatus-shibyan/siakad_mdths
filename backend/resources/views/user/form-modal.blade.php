...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md transition-opacity duration-300 animate-in fade-in"
    ...
    <div
        ...
        <form id="userForm" action="{{ $actionUrl }}" method="POST"
            class="overflow-y-auto p-6 space-y-4 flex-1 custom-scrollbar">
            @csrf
            @if ($isEdit)
                @method('PUT')
            @endif
            ...
            <div id="formErrorAlert"
                class="hidden p-3.5 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-400 text-xs font-bold space-y-1">
            </div>
